...
extern "C" {
  ...
  int simulation_step(kite* k, const double step, double force_){
    auto status=k->update_state(step, force_);
    return status;
  }
  ...
  int simulate(kite* k, const int integration_steps, const double step, double force_){
    bool continuation=true;
    int status;
    int i=0;
    while(continuation && i<integration_steps) {
        status=k->update_state(step,force_);
  ...
  vect getaccelerations(kite* k, double force_){
    return k->get_accelerations(force_).second;
  }

  double getvelocity(kite* k){
    return k->get_velocity();
    
  } 
  
  
   double getforce_r(kite* k){
    return k->get_force_r();
  } 
  double getreward(kite* k, double force_){
    return (k->compute_power(force_))/3600000;
  }
  
  double get_tension(kite* k, double force_){ 
    return k->compute_tension(force_);
   }
