...
public:
 ...
 // kite(vect initial_position, vect initial_velocity, Wind3d* wind_ptr, ): position{initial_position}, velocity{initial_velocity}, wind{wind_ptr}, C_l{0.2}, C_d{0.05}, psi{0} {
  kite(vect initial_position, vect initial_velocity, Wind3d* wind_ptr, double ll, double dd, double p): position{initial_position}, velocity{initial_velocity}, wind{wind_ptr}, C_l{ll}, C_d{dd}, psi{p} {
  ...
  int update_state(const double step, double force_){
    ...
    time+=step;
    auto accel=get_accelerations(force_);
    if(!accel.first){
      return 2;
    }
  ...
  double compute_power(double force_){
    //vect f=compute_force().second;
    //vect t=tension(f);
    
    if(velocity.r>0) return -velocity.r*force_;
    return velocity.r*force_;
  }
  double compute_tension(double force_){
    vect f=compute_force().second;
    vect t = tension(f, force_);
    return t.r;
  }
  ...
  std::pair<bool, vect> get_accelerations(double force_){
    std::pair<bool, vect> f=compute_force();
    if(!f.first){
      return {false, vect{}};
    }
    vect force=f.second;
    vect t=tension(force,force_);
    force-=t;
  ...
  std::pair<bool, vect> compute_force() const{
    ...
    vect tension(const vect& forces, double force_) const{
        auto num=M*er*forces.r+2*m*10*velocity.r/er+2*m*force_*er;
